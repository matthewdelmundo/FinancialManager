...
###########################################################
# Financial Manager Implementation
# Repurposed for use in financial manager
###########################################################
...
class CalendarWidget(RelativeLayout):
    ...
    def __init__(self, database, current_date, as_popup=False, **kwargs):
    ...
    def init_ui(self):
        ...
        self.create_month_scr(self.quarter[1], self.quarter_nums[1][0],
                              self.quarter_nums[1][1], toogle_today=True)
    ...
    def create_month_scr(self, month, year_num, month_num, toogle_today=False):
        ...
        for week in month:
            for day in week:
                if day[2] != 0:
                    # Adds an indicator for dates with data in them
                    if self.database.is_date_in_database([day[0], month_num, year_num]):
                        if day[1] >= 5:  # weekends
                            tbtn = DataWeekendButton(text=str(day[0]))
                        else:  # work days
                            tbtn = DataDayButton(text=str(day[0]))
                    else:
                        # Dates without any data
                        if day[1] >= 5:  # weekends
                    ...
                    if day[1] >= 5:  # weekends
                        tbtn = NonMonthWeekendButton(text=str(day[0]))
                    else:  # work days
                        tbtn = NonMonthButton(text=str(day[0]))
    ...
    def get_btn_value(self, inst):
        ...
        self.current_date = self.active_date
    ...
    def go_prev(self, inst):
        ...
        if not self.sm.has_screen(prev_scr_name):
            self.create_month_scr(self.quarter[0], self.quarter_nums[0][0],
    ...
    def go_next(self, inst):
        ...
        if not self.sm.has_screen(next_scr_name):
            self.create_month_scr(self.quarter[2], self.quarter_nums[2][0],
...
class NonMonthButton(ToggleButton):
    pass


class NonMonthWeekendButton(NonMonthButton):
    pass


class DataDayButton(ToggleButton):
    pass


class DataWeekendButton(DataDayButton):
    pass
